...
import { chunk, isValidAddress } from "./utils";
...
export const initReservesByHelper = async (
  ...
  bTokenImplementationAddress = (await hre.deployments.get(BTOKEN_IMPL_ID))
    .address;
  ...
  for (let i = 0; i < reserveSymbols.length; i++) {
    ...
    if (aTokenType[reserveSymbols[i]] === "generic") {
      aTokenToUse = bTokenImplementationAddress;
    } else {
      aTokenToUse = delegationAwarebTokenImplementationAddress;
    }

    initInputParams.push({
      bTokenImpl: aTokenToUse,
...
export const configureReservesByHelper = async (
  ...
  )) as unknown as ProtocolDataProvider;
