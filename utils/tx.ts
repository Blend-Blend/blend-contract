...
export const getProxyAdminBySlot = async (proxyAddress: tEthereumAddress) => {
  const proxyAdminSlot = await hre.ethers.provider.getStorage(
    proxyAddress,
  ...
  return hre.ethers.getAddress(
    abiCoder.decode(["address"], proxyAdminSlot).toString()
  );
};
...
export const getProxyImplementationBySlot = async (
  ...
  const proxyImplementationSlot = await hre.ethers.provider.getStorage(
    proxyAddress,
  ...
  const abiCoder = new hre.ethers.AbiCoder();
  return hre.ethers.getAddress(
    abiCoder.decode(["address"], proxyImplementationSlot)
      .toString()
  );
};
