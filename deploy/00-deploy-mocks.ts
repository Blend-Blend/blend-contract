...
import { TestnetMarket } from "../utils/testnet";
import { getReserveAddresses, getSymbolsByPrefix, isIncentivesEnabled } from "../utils/market-config-helpers";
import { eNetwork } from "../utils/types";
import { TESTNET_PRICE_AGGR_PREFIX, TESTNET_REWARD_TOKEN_PREFIX } from "../utils/deploy-ids";
...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const faucet = await getDeployedContract("Faucet", "Faucet");
  
  // Deploy Testnet tokens
  const reservesConfig = TestnetMarket.ReservesConfig;
  ...
  if (hre.network.name == "hardhat") {
    await deploy("WETH9Mock", {
      ...
      from: deployer,
      args: [
        TestnetMarket.WrappedNativeTokenSymbol,
        TestnetMarket.WrappedNativeTokenSymbol,
        faucet.target,
      ],
    }).then((res) => {
      console.log("WETH9Mock deployed to: %s, %s", res.address, res.newlyDeployed);
    });
  }
  ...
  if (isIncentivesEnabled(TestnetMarket)) {

    const rewardSymbols: string[] = Object.keys(
      TestnetMarket.IncentivesConfig.rewards[network] || {}
    );
